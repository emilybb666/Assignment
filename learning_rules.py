...
class AdamLearningRule(GradientDescentLearningRule):
    ...
    def reset(self):
        ...
        """
        for moms_1 in zip(self.moms_1):
            moms_1 *= 0.
        for moms_2 in zip(self.moms_2):
            moms_2 *= 0.
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, moms_1, moms_2, grad in zip(
            ...
            step_count += 1
            moms_1 = self.beta_1*moms_1+(1-self.beta_1)*grad
            moms_2 = self.beta_2*moms_2+(1-self.beta_2)*grad**2
            moms_1_t = moms_1/(1-self.beta_1**step_count)
            moms_2_t = moms_2/(1-self.beta_2**step_count)
            param -= self.learning_rate*moms_1_t/(moms_2_t+self.epsilon)**0.5
...
class AdamLearningRuleWithWeightDecay(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, moms_1, moms_2, grad in zip(
            ...
            moms_1_t = moms_1/(1-self.beta_1**step_count)
            moms_2_t = moms_2/(1-self.beta_2**step_count)
            param -= (self.learning_rate/self.initial_learning_rate)*(self.initial_learning_rate*moms_1_t/(moms_2_t+self.epsilon)**0.5+self.weight_decay*param)
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, moms, grad in zip(
            self.params, self.moms, grads_wrt_params):
            moms *= self.beta
            moms += (1-self.beta)*grad**2
            param -= self.learning_rate*grad/(moms+self.epsilon)**0.5
